...
Get Posts
----------------------------------------------------------------------------- */
export async function getPosts({limit,include,fields,filter,formats,order,page} = {}) {
  const api = getContentApi()

  const config = {
    limit: limit ? limit : THEME_CONFIG.POSTS_PER_PAGE,
...
export async function getMemberNewsletters() {
  const member = await fetch(`${themeGlobal.siteUrl}/members/api/member/`);
...
export async function updateMemberNewsletters(newsletters) {
  try {
    const response = await fetch(`${themeGlobal.siteUrl}/members/api/member/`, {
